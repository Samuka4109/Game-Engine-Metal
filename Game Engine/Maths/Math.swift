...
public var X_ASIS: SIMD3<Float>{
    return SIMD3(1,0,0)
}
public var Y_ASIS: SIMD3<Float>{
    return SIMD3(0,1,0)
}
public var Z_ASIS: SIMD3<Float>{
    return SIMD3(0,0,1)
}
...
extension matrix_float4x4{
    ...
    static func perspective(degreesFov: Float, aspectRadio: Float, near: Float, far: Float) -> matrix_float4x4{
        let fov = degreesFov.toRadians
        
        let t: Float = tan(fov / 2)
        
        let x: Float = 1 / (aspectRadio * t)
        let y: Float = 1 / t
